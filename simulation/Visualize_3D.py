...
def compute_joint_positions(foot_targets):
    """
...
def run_simulation(step_length=STEP_LENGTH,
    ...
    def update(frame):
        ...
        # 1. Gait generator → foot targets (hip frame)
        foot_targets = gait.get_foot_targets(t)

        # 2. IK + FK → world-frame joint positions
        joint_positions = compute_joint_positions(foot_targets)
        ...
        for leg in LEG_NAMES:
            ...
            angle_lines.append(
                f"{leg}: {state} | "
                f"θ0={int(np.degrees(t0))} "
                f"θ1={int(np.degrees(t1))} "
                f"θ2={int(np.degrees(t2))}"
            )
        ...
        return list(artists.values()) + [phase_txt, time_txt]
